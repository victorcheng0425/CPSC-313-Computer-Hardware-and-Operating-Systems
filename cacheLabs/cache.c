                   ...
                   unsigned int associativity, int policies)
{
    ...
    //get_shift_and_mask(int value, unsigned int *shift, uint64_t *mask, int init_shift)
...
/*
 * Frees all memory allocated for a cache set with the given associativity.
 */
static void cache_set_free(cache_set_t *cache_set, unsigned int associativity)
{
    ...
    for(i = 0; i < associativity; i++)
    {
        free(cache_set->cache_lines[i]);
    }
    free(cache_set->cache_lines);
...
void cache_free(cache_t *cache)
{
    ...
    int i;
    for(i = 0; i < cache->num_sets; i++) {
        cache_set_free(&cache->sets[i], cache->associativity);
    }
    free(cache->sets);
    free(cache);
}
...
{
    /* TO BE COMPLETED BY THE STUDENT */
    return (cache_line->is_valid == 1 && tag == cache_line->tag);
...
static long cache_line_retrieve_data(cache_line_t *cache_line, size_t offset)
{
    ...
    //uint64_t* ptr = (uint64_t*)(&cache_line->data[offset]);
    //return *ptr;
    uint64_t a = 0;
    uint64_t value = 0;
    for (int i = 0; i <=7; i++){
        a = cache_line->data[offset+i];
        value += (a << (i*8));
    }
    return value;
    //return 0; // Added to remove warning; remove once function is implemented.
...
static cache_line_t *cache_set_find_matching_line(cache_t *cache, cache_set_t *cache_set,
   ...
   */
    ...
    //int j;
    /*
    for (i = 0; j < cache->num_sets; i++) {
        for (j = 0; j < cache->associativity; j++) {
            if ((cache_line_check_validity_and_tag(cache_set->cache_lines[j], tag)) &&
                (cache_line_check_validity_and_tag(cache->sets[i].cache_lines[j], tag))) {
                cache_line_t *line = cache_line_make_mru(cache_set, j);
                return line;
            }
        }
    }
    ...
    for (i = 0; i < cache->associativity; i++) {
        if (cache_line_check_validity_and_tag(cache_set->cache_lines[i], tag)){
            cache_line_t *line = cache_line_make_mru(cache_set, i);
            return line;
        }
    }
 ...
 * the cache's replacement policy. For testing purposes, we require
 * you to use rand() if you need to generate random numbers.
 */
static cache_line_t *find_available_cache_line(cache_t *cache, cache_set_t *cache_set) {
   ...
   */
    ...
    if ((cache->policies & CACHE_REPLACEMENTPOLICY_MASK) == CACHE_REPLACEMENTPOLICY_RANDOM){
        //return cache_line_make_mru(cache_set, rand() % ((cache->associativity)));
        cache_line_t *temp = cache_set->cache_lines[rand() % ((cache->associativity))];
        return temp;
    }
...
long cache_read(cache_t *cache, long *address)
{
    //srand(time(0));
    /* TO BE COMPLETED BY THE STUDENT */
    //printf("Address: %ln", address);
    uintptr_t addr = (uintptr_t) address;
    uintptr_t offset = addr & cache->block_offset_mask;
    addr = (uintptr_t)address;
    uintptr_t lineOrSet = (addr >> cache->cache_index_shift) & cache->cache_index_mask;
    addr = (uintptr_t)address;
    uintptr_t tag = (addr >> cache->tag_shift);
    cache_set_t *set = &(cache->sets[lineOrSet]);
